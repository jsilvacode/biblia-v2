...
function assertRequiredMetadata(metadata) {
  ...
  if (missing.length) throw new Error(`Falta metadata requerida: ${missing.join(', ')}.`)
...
export function parseStudyLesson(raw) {
  ...
  for (const line of body.split('\n')) {
    ...
    if (sectionHeading) {
      ...
      currentSection = { id: `s-${lessonPrefix}-${titleSlug}`, title, questions: [] }
...
export function validateStudyLessons(studyLessons, manifest) {
  const errors = []
  ...
  if (studyLessons.length !== manifest.length) {
    errors.push(`El manifiesto declara ${manifest.length} lecciones, pero se procesaron ${studyLessons.length}.`)
  }
  ...
  const totals = { lessons: studyLessons.length, questions: questionCount, references: referenceCount }
