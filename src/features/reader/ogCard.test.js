...
describe('entrega de tarjetas sociales', () => {
  it.each([
    ...
    const response = await ogCard.fetch(new Request(`${origin}/api/og-card?${query}&card=10`))
  ...
  it('HEAD declara el mismo tamaño que GET y no envía cuerpo', async () => {
    const url = `${origin}/api/og-card?type=verse&book=43&chapter=6&verse=37&v=rva2015&card=10`
