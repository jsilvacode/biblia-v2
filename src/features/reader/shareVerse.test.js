...
describe('shareVerse', () => {
  it('builds a human reader URL that preserves version, locale and a verse range', () => {
    expect(createVerseShareUrl({
      ...
      verseEnd: 4,
      versionId: 'nbla',
    })).toBe('https://www.santabiblia.cloud/read/1/46/3?v=nbla&end=4&lang=pt-BR&share=11')
  })
  ...
  it('keeps localhost URLs available while developing', () => {
    expect(createVerseShareUrl({
      ...
      verse: 16,
      versionId: 'nbla',
    })).toBe('http://127.0.0.1:5173/read/43/3/16?v=nbla&share=11')
  })
  ...
  it('shares one detectable canonical URL so messaging apps can render its card', async () => {
    ...
    const data = createVerseShareData({ reference: 'Juan 3:16', text: 'Porque de tal manera…', url: 'https://www.santabiblia.cloud/read/43/3/16?v=nbla&share=11', version: 'NBLA' })

    await expect(shareVerse(data)).resolves.toBe('shared')
    expect(share).toHaveBeenCalledWith({
      title: 'Juan 3:16 · NBLA',
      text: 'https://www.santabiblia.cloud/read/43/3/16?v=nbla&share=11',
    })
    expect(fetchMock).toHaveBeenCalledWith(
      new URL('https://www.santabiblia.cloud/api/og-card?type=verse&book=43&chapter=3&verse=16&v=nbla&card=11'),
