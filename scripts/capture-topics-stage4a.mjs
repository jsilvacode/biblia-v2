...
const outputDirectory = resolve('docs/implementation/evidence/etapa-4a-guia-tematica/final')
const states = [
...
async function capture(browser, state) {
  ...
  await page.getByRole('heading', { name: '¿Qué necesitas hoy?' }).waitFor()
  await page.screenshot({ fullPage: true, path: resolve(outputDirectory, `guia-tematica-${state.name}.jpg`), quality: 84, type: 'jpeg' })
  await context.close()
}
